...
export let pairs = writable(
  pairsFromStorage || [{ locked: false, members: [] }]
);
...
export function generatePairs() {
  ...
  for (let lockedPair of tempPairs) {
    lockedPair.members.forEach((member) => {
      temp = temp.filter(
        (tempMember) => member.originalId !== tempMember.originalId
      );
    });
  }
  while (temp.length > 0) {
...
export function addMember(name) {
  ...
  members.update((members) => [
    { id: id, name: name, originalId: id, isCopy: false },
    ...members,
  ]);
}
...
export function removeMember(memberToRemove) {
  ...
  members.update((members) =>
    members.filter((member) => memberToRemove.id !== member.id)
  );
}
...
export function removeMemberFromPair(memberToRemove) {
  pairs.update((pairs) => {
    const newPairs = pairs
      .map((pair) => ({
        ...pair,
        members: pair.members.filter(
          (member) => member.originalId !== memberToRemove.originalId
        ),
      }))
...
export function updateMemberName(member, newName) {
  ...
  pairs.update((pairs) =>
    pairs.map((pair) => {
      let memberInPair = pair.members.find(
        (m) => m.originalId === member.originalId
      );
      if (memberInPair) {
